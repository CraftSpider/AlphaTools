...
void test_btol() {
    ...
    ASSERT(btol<Endian::BIG>(test1) == 0x0807060504030201);
    ASSERT(btol<Endian::LITTLE>(test1) == 0x0102030405060708);
    
    char test2[8] = {8, 7, 6, 5, 4, 3, 2, 1};
    ASSERT(btol<Endian::BIG>(test2) == 0x0807060504030201);
    ASSERT(btol<Endian::LITTLE>(test2) == 0x0102030405060708);
}

void test_btoi() {
    uchar test1[4] = {8, 7, 6, 5};
    ASSERT(btoi<Endian::BIG>(test1) == 0x08070605);
    ASSERT(btoi<Endian::LITTLE>(test1) == 0x05060708);
    
    char test2[4] = {8, 7, 6, 5};
    ASSERT(btoi<Endian::BIG>(test2) == 0x08070605);
    ASSERT(btoi<Endian::LITTLE>(test2) == 0x05060708);
}

void test_btos() {
    uchar test1[2] = {8, 7};
    ASSERT(btos<Endian::BIG>(test1) == 0x0807);
    ASSERT(btos<Endian::LITTLE>(test1) == 0x0708);
    
    char test2[2] = {8, 7};
    ASSERT(btos<Endian::BIG>(test2) == 0x0807);
    ASSERT(btos<Endian::LITTLE>(test2) == 0x0708);
}

void test_btoc() {
    uchar test1[1] = {8};
    ASSERT(btoc<Endian::BIG>(test1) == 0x08);
    ASSERT(btoc<Endian::LITTLE>(test1) == 0x08);
    
    char test2[1] = {1};
    ASSERT(btoc<Endian::BIG>(test2) == 0x01);
    ASSERT(btoc<Endian::LITTLE>(test2) == 0x01);
}
...
void test_ctob() {
    ...
    temp = ctob<Endian::BIG>(test1, 1);
    ASSERT(compare(temp, expected1, 1));
    delete[] temp;
    temp = ctob<Endian::LITTLE>(test1, 1);
    ASSERT(compare(temp, reverse(expected1, 1), 1));
    delete[] temp;
    
    uchar test2 = 0x0;
    uchar expected2[1] = {0};
    temp = ctob<Endian::BIG>(test2, 1);
    ASSERT(compare(temp, expected2, 1));
    delete[] temp;
    temp = ctob<Endian::LITTLE>(test2, 1);
    ...
    temp2 = ctob<Endian::BIG>(test3, 1);
    ASSERT(compare(temp2, expected3, 1));
    delete[] temp2;
    temp2 = ctob<Endian::LITTLE>(test3, 1);
    ASSERT(compare(temp2, reverse(expected3, 1), 1));
    delete[] temp2;
    
    char test4 = 0x0;
    char expected4[1] = {0};
    temp2 = ctob<Endian::BIG>(test4, 1);
    ASSERT(compare(temp2, expected4, 1));
    delete[] temp2;
    temp2 = ctob<Endian::LITTLE>(test4, 1);
